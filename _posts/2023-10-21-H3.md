...
bassu N m non ha una forma definita
The theoretical limit distribution of security scores as N (systems) and M (samples) increases could be approximated using probability theory, anche perchè conoscimao la relazione tra p and the outcomes of each attack.
Depending on the characteristics (high N and M) of the simulation, we expect a limiting distribution that approaches a normal distribution (bell curve).

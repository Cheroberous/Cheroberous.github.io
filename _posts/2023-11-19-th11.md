...
The stochastic process $ S:=\left(S_n\right)_{n \in N} $ is known as a random walk. Define the diffusively rescaled random walk (partial-sum process) by $W^n(t) := \frac{S_{\lfloor nt \rfloor}}{\sqrt{n}}, \qquad t\in [0,1]$.
...
Donsker's invariance principle[1][2] extends this convergence to the whole function $ W^{(n)}:=(W^{(n)}(t))_{t\in [0,1]}$. More precisely, in its modern form, Donsker's invariance principle states that: As random variables taking values in the Skorokhod space 

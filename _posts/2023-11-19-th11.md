...
The stochastic process S:= $ \left(S_{n}\right)_{n \in N} $ is known as a random walk. Define the diffusively rescaled random walk (partial-sum process) by $W^n(t) := \frac{S_{\lfloor nt \rfloor}}{\sqrt{n}}, \qquad t\in [0,1]$.

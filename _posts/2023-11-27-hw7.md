...
## Implementation
Starting from the homework 3 i have kept the plot function and make some sligth changes as in the max calculation to reproportion the linechart:
...
after the array of arrays (of trajectories) is generated the different points will simply read and draw by the **draw()** function:
```javascript
simulate() {
...
```
Most of the distribution formula came from **your page** <https://www.datatime.eu/public/cybersecurity/jsTutorial/23_EulerMaruyama.html> the remaining were esxtracted from wikipedia-related pages. <br>
